...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	if larkKey, err := secretbox.LoadKey("MULTICA_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// velafi-lark-inbox-pack (backport #3919): DM inbox notifications
				// (assignments/status/comments/@mentions/agent activity) to members
				// bound to the relevant agent's bot, with a workspace-configured
				// fallback agent for events with no agent bot. Best-effort;
				// subscribes to inbox:new.
	...
	// velafi-auth-fork-pack: Velafi SSO (Authentik/Lark) login + health.
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceMemberFromURL(queries, "id"))
					// velafi-org-invite-pack: roster-backed quick-add + directory search.
				...
				// inbox-notifier agent is a workspace-level mutating op. Its
				// handler does NOT self-gate (no canManageAgent) — it relies on
				// route-level role gating — so it stays in an owner/admin group.
				// Folding it into the member group above would let any member
				// reconfigure Lark inbox notifications (security regression).
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceRoleFromURL(queries, "id", "owner", "admin"))
