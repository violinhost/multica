...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	// Auth (public) — per-IP rate limiting (upstream v0.3.2) + fork OIDC routes.
	if rdb == nil {
	...
	r.With(authRL).Post("/auth/oidc", h.OIDCLogin)
	r.With(authRL).Get("/api/oidc/health", h.OIDCHealth)
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					r.Get("/velafi/directory/search", h.VelafiDirectorySearch)
				...
				r.Group(func(r chi.Router) {
					...
					// Member-visible (read-only), same rationale as the
					// installations listing above.
				...
				// velafi-lark-inbox-pack: setting the workspace-wide fallback
				// inbox-notifier agent is a workspace-level mutating op. Unlike
				// the per-agent endpoints above, its handler does NOT self-gate
				// (no canManageAgent) — it relies on route-level role gating —
				// so it stays in an owner/admin group. Folding it into the
				// member group above would let any member reconfigure Lark inbox
				// notifications (security regression).
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceRoleFromURL(queries, "id", "owner", "admin"))
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/projects", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					r.Get("/resources/{resourceId}/ci-profile", h.GetCIRepositoryProfileDiscovery)
					r.Post("/resources/{resourceId}/ci-profile", h.RegisterCIRepositoryProfile)
					r.Post("/resources/{resourceId}/ci-profile/enable", h.EnableCIRepositoryProfile)
					r.Post("/resources/{resourceId}/ci-profile/disable", h.DisableCIRepositoryProfile)
				})
			})
