...
var workspaceDeletionManifest = map[string]workspaceDeleteAction{
	...
	// velafi-lark-inbox-pack: the two Lark inbox delivery ledgers (migrations
	// 122/123). Opaque-key dedup rows with no FK, scoped per workspace via the
	// installation they were claimed under — same lifecycle as channel_*.
	"lark_inbox_comment_delivery":      workspaceDelete,
	"lark_inbox_notification_delivery": workspaceDelete,
	...
	"issue_orchestration_projection":         workspaceDelete,
	"issue_orchestration_projection_receipt": workspaceDelete,
}
