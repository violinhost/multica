...
type Receipt struct {
	DispatchID, WorkspaceID, ProjectID, ParentIssueID string `json:"dispatch_id,omitempty" json:"-"`
	ActionKey                                         string `json:"action_key"`
...
func (s *Service) Reconcile(ctx context.Context) {
	...
	rows, err := s.DB.Query(ctx, `SELECT o.id, o.task_id, o.runtime_id FROM managed_action_outbox o WHERE o.status='pending' ORDER BY o.created_at LIMIT 100`)
	if err != nil {
		return
	}
	defer rows.Close()
	for rows.Next() {
		...
		_, _ = s.DB.Exec(ctx, `UPDATE managed_action_outbox SET status='delivered', attempts=attempts+1, delivered_at=now(), updated_at=now() WHERE id=$1 AND status='pending'`, outboxID)
	}
}
...
func validateRequest(req Request, spec Spec) error {
	...
	if json.Unmarshal(req.RevisionFacts, &revision) != nil || len(revision) == 0 {
		return ErrInvalidRequest
	}
	return nil
}
