...
var legacyDuplicateMigrationStems = map[string][]string{
	...
	// velafi: 068_user_external_identity (OIDC/Authentik) is a carried
	// velafi-only migration that shares the 068 prefix with upstream's
	// 068_timeline_keyset_index. prod records migrations by full filename, so
	// renumbering would re-run it and break rollback; keep both.
	"068": {"068_timeline_keyset_index", "068_user_external_identity"},
	...
	// velafi: 122_lark_inbox_notification_delivery and
	// 123_lark_inbox_comment_delivery are carried velafi-only migrations
	// (velafi-lark-inbox-pack) sharing prefixes with upstream migrations. prod
	// records migrations by full filename, so renumbering would re-run them and
	// conflict on table creation; keep both.
	"122": {"122_lark_chat_session_binding_thread_reply", "122_lark_inbox_notification_delivery", "122_task_handoff_note"},
	"123": {"123_issue_stage", "123_lark_inbox_comment_delivery"},
